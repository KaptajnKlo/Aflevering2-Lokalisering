...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    model.obj=pyomo.Objective(expr=(sum(model.Defterspørgsel2030[j]*model.travel_times[i][j]*model.x[i,j] for i in model.RangeAntalKommuner for j in model.RangeAntalKommuner)), sense=pyomo.minimize)
    ...
    for i in model.RangeAntalKommuner:
        model.Kapacitet2022.add(expr=(
                    sum(model.Defterspørgsel2022[j] * model.x[i, j] for j in model.RangeAntalKommuner) <=
                    model.basiskapacitet[i] * model.y[i] + 1000 * model.z[i]))
    # Kapacitetsbegrænsning 2030
    model.Kapacitet2030 = pyomo.ConstraintList()
    for i in model.RangeAntalKommuner:
        model.Kapacitet2030.add(expr=(
                    sum(model.Defterspørgsel2030[j] * model.x[i, j] for j in model.RangeAntalKommuner) <=
                    model.basiskapacitet[i] * model.y[i] + 1000 * model.z[i]))
    ...
    model.BudgetGrænse = pyomo.Constraint(
        expr=(sum(model.fasteomkostninger[i] * model.y[i] * model.mia for i in model.RangeAntalKommuner) + sum(
            model.ekstraomkostninger[i] * model.z[i] for i in model.RangeAntalKommuner) <= model.budget_begrænsning)
    )

    # Åben specifikt x antal sygehuse
    model.ÅbenAntalSygehuse = pyomo.Constraint(
        expr=sum(model.y[i] for i in model.RangeAntalKommuner) == model.AntalSygehuse)
    return model
