...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    #Define variables
    model.x=pyomo.Var(model.RangeAntalKommuner,model.RangeAntalKommuner, within=pyomo.NonNegativeReals)
    ...
    model.obj=pyomo.Objective(expr=(sum(model.distances[i][j]*model.x[i,j] for i in model.RangeAntalKommuner for j in model.RangeAntalKommuner)), sense=pyomo.minimize)

    #### ------------- Constraints -------------- ####
    # Al efterspørgsel skal efterkommes 100%
        ...
        if pyomo.value(model.y[i]) == 1:
            ...
            for j in model.RangeAntalKommuner:
                if pyomo.value(model.x[i, j]) > 0:
                    print(model.kommuner[j], "(", pyomo.value(model.x[i, j])*100, "%)", end=',')
            print('\n')
