...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    #### ------------- Constraints -------------- ####
    #Definition af rhoMax
    model.rhoMaxDef = pyomo.ConstraintList()
    for j in model.RangeAntalKommuner:
        model.rhoMaxDef.add(expr=sum(model.distances[i][j]*model.x[i, j] for i in model.RangeAntalKommuner) <= model.rhoMax)
    ...
    model.Repræsentant = pyomo.ConstraintList()
    for i in model.RangeAntalKommuner:
        model.Repræsentant.add(expr=model.x[i,i] == model.y[i])
